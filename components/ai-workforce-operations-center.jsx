...
import {
  Activity, AlertTriangle, Bot, BrainCircuit, CheckCircle2, ChevronRight,
...
export default function AIWorkforceOperationsCenter() {
  ...
  const [selectedAgent, setSelectedAgent] = useState("Pilot");
  ...
  const selectedAgentProfile = agents[selectedAgent];
  const selectedAgentAssignments = assignments.filter((item) => item.agent_code === selectedAgent);
  const selectedAgentRecommendations = recommendations.filter((item) => item.agent_code === selectedAgent);
  const selectedAgentCanAnalyze = selectedAgentAssignments.some((item) => !["approved", "analyzing"].includes(item.assignment_status));
  ...
  function openAgentEvent(eventId, targetId = "event-workspace") {
    const event = events.find((item) => item.id === eventId);
    setSelectedEventId(eventId);
    if (event) setNotice(`${selectedAgent} context opened for ${event.event_title}.`);
    scrollToSection(targetId);
  }

  function reviewAgent(name) {
    const recommendation = recommendations.find((item) => item.agent_code === name);
    const assignment = assignments.find((item) => item.agent_code === name);
    setSelectedAgent(name);
    if (recommendation?.event_id || assignment?.event_id) setSelectedEventId(recommendation?.event_id || assignment.event_id);
    setNotice(`${name} workspace opened. Review routed work, prior recommendations, or request a new supervised analysis.`);
    scrollToSection("agent-review");
  }

  async function seekAgentRecommendation(name) {
    const assignment = assignments.find((item) => item.agent_code === name && item.event_id === selectedEvent?.id && !["approved", "analyzing"].includes(item.assignment_status))
      || assignments.find((item) => item.agent_code === name && !["approved", "analyzing"].includes(item.assignment_status));
    setSelectedAgent(name);
    if (!assignment) {
      setNotice(`${name} has no routed event awaiting analysis. Select or submit a relevant Northstar event first.`);
      scrollToSection("agent-review");
      return;
    }
    setSelectedEventId(assignment.event_id);
    scrollToSection("recommendations");
    await runAgent(assignment);
  }
  ...
  return <main className="wf-shell">
    ...
    <section className="workspace-grid" id="event-workspace">
    ...
    <section className="panel workforce-panel">
      <div className="section-title"><div><small>AI WORKFORCE DIRECTORY</small><h2>Open a specialist, review its work, or seek a recommendation</h2><p>Each agent has a defined operating specialty. Select an agent to review routed assignments and prior recommendations, then request analysis against an authorized Northstar event.</p></div><Users size={25}/></div>
      <div className="agent-grid">{Object.entries(agents).map(([name,profile])=>{const assigned=assignments.filter((item)=>item.agent_code===name);const waiting=assigned.filter((item)=>["queued","analyzing","awaiting_human","recommendation_ready"].includes(item.assignment_status)).length;const approved=assigned.filter((item)=>item.assignment_status==="approved").length;const canAnalyze=assigned.some((item)=>!["approved","analyzing"].includes(item.assignment_status));return <article className={selectedAgent===name ? "aiwf-agent-card selected" : "aiwf-agent-card"} key={name}><div><span className="agent-mark large">{profile.initials}</span><span><strong>{name}</strong><small>{profile.role}</small></span></div><p>{profile.focus}</p><footer><span><small>Assigned</small><strong>{assigned.length}</strong></span><span><small>Waiting</small><strong>{waiting}</strong></span><span><small>Approved</small><strong>{approved}</strong></span></footer><div className="agent-card-actions no-print"><button onClick={()=>reviewAgent(name)}>Review agent <ChevronRight size={14}/></button><button className="primary" onClick={()=>seekAgentRecommendation(name)} disabled={!canAnalyze || Boolean(busy)}>Seek recommendation <Send size={14}/></button></div></article>})}</div>

      <article className="agent-review" id="agent-review">
        <header className="agent-review-head">
          <div className="agent-identity"><span className="agent-mark large">{selectedAgentProfile.initials}</span><span><small>SELECTED AI SPECIALIST</small><h3>{selectedAgent}</h3><strong>{selectedAgentProfile.role}</strong><p>{selectedAgentProfile.focus}</p></span></div>
          <div className="agent-review-head-actions no-print"><button onClick={()=>scrollToSection("event-workspace")}>Choose event <Network size={15}/></button><button className="primary" onClick={()=>seekAgentRecommendation(selectedAgent)} disabled={!selectedAgentCanAnalyze || Boolean(busy)}>{busy ? <RefreshCw size={15}/> : <Sparkles size={15}/>}Seek {selectedAgent} recommendation</button></div>
        </header>

        <div className="agent-review-body">
          <section className="agent-review-column">
            <div className="agent-review-title"><div><small>ROUTED WORK</small><h4>Assignments for {selectedAgent}</h4></div><span>{selectedAgentAssignments.length}</span></div>
            <div className="agent-review-list">
              {!selectedAgentAssignments.length && <div className="agent-review-empty"><Route size={18}/><span>No events are currently routed to this specialist.</span></div>}
              {selectedAgentAssignments.slice(0,6).map((assignment)=>{const event=events.find((item)=>item.id===assignment.event_id);return <button className="agent-review-item" key={assignment.id} onClick={()=>openAgentEvent(assignment.event_id)}><span><strong>{event?.event_title || "Connected Northstar event"}</strong><small>{assignment.assignment_reason}</small></span><em className={`status ${assignment.assignment_status}`}>{titleCase(assignment.assignment_status)}</em><ChevronRight size={15}/></button>})}
            </div>
          </section>

          <section className="agent-review-column">
            <div className="agent-review-title"><div><small>RECOMMENDATION HISTORY</small><h4>{selectedAgent} recommendations</h4></div><span>{selectedAgentRecommendations.length}</span></div>
            <div className="agent-review-list">
              {!selectedAgentRecommendations.length && <div className="agent-review-empty"><Bot size={18}/><span>No recommendations have been prepared by this specialist yet.</span></div>}
              {selectedAgentRecommendations.slice(0,6).map((recommendation)=><button className="agent-review-item" key={recommendation.id} onClick={()=>openAgentEvent(recommendation.event_id,"recommendations")}><span><strong>{recommendation.recommendation_title}</strong><small>{recommendation.model_mode === "ai" ? "Model-assisted" : "Rules-based"} · Confidence {recommendation.confidence}%</small></span><em className={`status ${recommendation.recommendation_status}`}>{titleCase(recommendation.recommendation_status)}</em><ChevronRight size={15}/></button>)}
            </div>
          </section>
        </div>
      </article>
