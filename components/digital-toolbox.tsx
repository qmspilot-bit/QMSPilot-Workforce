...
const tools: Tool[] = [
  { id: "capa", title: "Corrective Action & CAPA", description: "Contain, investigate, assign, verify effectiveness, and submit the controlled record to Northstar.", status: "live", icon: ClipboardCheck },
  { id: "ncr", title: "Nonconformance Report", description: "Capture defects, evidence, disposition, risk, and escalation into the shared quality record.", status: "coming", icon: FileSearch },
  { id: "audit", title: "6S & Process Audits", description: "Run observable audits, assign corrective actions, and track sustained compliance.", status: "coming", icon: ShieldCheck },
  { id: "more", title: "More Northstar Apps", description: "Training, calibration, shipping evidence, COPQ, warranty, layered audits, and more.", status: "coming", icon: Boxes },
];
...
export function DigitalToolbox() {
  ...
  useEffect(() => {
    ...
    if (!workspace) return;
    let target = document.getElementById("northstar-toolbox-mount");
    if (!target) {
      target = document.createElement("div");
      target.id = "northstar-toolbox-mount";
      const missionCard = workspace.querySelector(".mission-card");
      if (missionCard) workspace.insertBefore(target, missionCard);
      else workspace.appendChild(target);
    }
    setMount(target);
  }, []);
        ...
        {tools.map((tool) => {
          ...
          return (
            <article className={live ? "tool-tile tool-tile-live" : "tool-tile"} key={tool.id}>
              ...
              <div className="tool-tile-top"><span>{live ? "Connected" : "Roadmap"}</span>{live ? <i className="tool-live-dot" /> : <Construction size={15} />}</div>
              <h3>{tool.title}</h3>
              <p>{tool.description}</p>
              <button type="button" disabled={!live} onClick={live ? launchCapa : undefined}>{live ? <>Open CAPA <ArrowUpRight /></> : "Coming to Northstar"}</button>
            </article>
          );
        })}
      </div>
  ...
  return (
    <>
      {mount ? createPortal(toolbox, mount) : null}
      {open && (
        <div className="app-launcher" role="dialog" aria-modal="true" aria-label="Corrective Action and CAPA application">
          <div className="app-launcher-shell">
            <header>
              <div><p>Northstar Application</p><strong>Corrective Action & CAPA</strong></div>
            ...
            <div className="launcher-frame-wrap">
              <iframe id="capaApplicationFrame" src={CAPA_URL} title="QMSPilot Corrective Action and CAPA" onLoad={sendContext} allow="camera; clipboard-write" />
            </div>
            <footer>
