import { ArrowRight, BarChart3, Boxes, BrainCircuit, ClipboardCheck, Gauge, HardHat, Settings, ShieldCheck, Truck, Users, Wrench } from "lucide-react";
  ...
  return (
    <main className="toolbox-shell">
      ...
      <section className="main">
        <header><div><small>QMSPILOT NORTHSTAR</small><strong>Workspaces</strong></div><span>Seven connected operating environments</span></header>
        <div className="content">
          ...
          <section className="grid workspace-grid">
            {workspaces.map(({title,description,href,icon:Icon}) => (
              <a className="tile featured" href={href} key={title}><div className="tile-top"><span><Icon size={25}/></span><em>READY</em></div><h2>{title}</h2><p>{description}</p><div className="open">Open workspace <ArrowRight size={16}/></div></a>
            ))}
          </section>
