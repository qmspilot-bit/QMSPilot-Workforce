...
import { OperationsFunctionPanel, deriveOperationalMetrics } from "./OperationsFunctionPanel";
import {
  bytes,
  iconMap,
  makeRecordId,
...
export function OperationsGuidedWorkflow({
  config,
  tool,
  onBack,
}: {
  ...
  const [meta, setMeta] = useState<RecordMeta>({
    ...
    site: "Primary Site",
    recordId: makeRecordId(config, tool),
    eventDate: today(),
  });
  ...
  const key = `qmspilot:guided:operations:${tool.id}`;
  ...
  const completion = useMemo(() => {
    const completionItems = [
      ...
      Boolean(meta.organization),
      Boolean(meta.site),
      Boolean(meta.recordId),
      Boolean(meta.eventDate),
      Boolean(owner),
      Boolean(due),
      Boolean(approver),
      Boolean(decision),
    ...
    return Math.round(completionItems.filter(Boolean).length / Math.max(completionItems.length, 1) * 100);
  ...
  useEffect(() => {
    try {
      ...
      setNotice("Saved operational draft restored. Reattach original files when working from a different browser session.");
    } catch {
  ...
  const applyScan = (result: ScanResult) => {
    ...
    setScanner(false);
    setNotice("Code captured and matching operational fields populated.");
  };
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        setEvidence(current => [...current, {
          id: uid(),
          kind: "audio",
          name: `operations-voice-note-${Date.now()}.webm`,
          size: blob.size,
          url: URL.createObjectURL(blob),
        }]);
        stream.getTracks().forEach(track => track.stop());
        setRecording(false);
        setNotice("Operational voice note attached.");
      };
  ...
  const captureLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(position => {
      ...
      setEvidence(current => [...current, { id: uid(), kind: "location", name: "Verified operational location", size: location.length, note: location }]);
  ...
  const saveDraft = () => {
    ...
    setNotice("Smart Operations draft saved in this browser.");
  };
  ...
  const submit = () => {
    if (validate().length) {
      setNotice("Submission blocked until operational facts, required evidence, closure gates, and approval are complete.");
      return;
    }
    const derived = deriveOperationalMetrics(tool.id, values, due);
    const record: SubmissionRecord = {
      schema: "qmspilot.northstar.smart-operations.v3",
    ...
    localStorage.setItem("qmspilot:northstar:guided-records", JSON.stringify([record, ...records].slice(0, 250)));
    localStorage.removeItem(key);
    window.dispatchEvent(new CustomEvent("qmspilot:record-submitted", { detail: record }));
    setSubmitted(record);
    setNotice("Specialized Smart Operations record submitted to Northstar.");
    window.scrollTo({ top: 0, behavior: "smooth" });
  };

  const assignAction = () => {
    if (!owner || !due) {
      setNotice("Enter an accountable owner and due date first.");
      return;
    }
    const action = {
      id: `ACT-${uid()}`,
      sourceRecord: meta.recordId,
    ...
    localStorage.setItem("qmspilot:northstar:assigned-actions", JSON.stringify([action, ...actions].slice(0, 250)));
    setNotice(`Operational action assigned to ${owner}.`);
  };
  ...
  const newRecord = () => {
    ...
    setOwner("");
    setDue("");
    setChecks(Object.fromEntries(tool.controls.map(control => [control, false])));
    setRole(tool.approvalRoles[0] || "Authorized Approver");
    setApprover("");
    setDecision("");
    setConditions("");
    evidence.forEach(item => item.url && URL.revokeObjectURL(item.url));
    ...
    setSubmitted(null);
    setStage(0);
    setNotice("New Smart Operations record started.");
  };
  ...
  const guidance = priority === "Critical"
    ...
    : tool.evidenceRequired && !evidence.length
      ...
      : completion < 50
        ? "Complete the operational facts before selecting the response and commitment."
        : "Verify the calculated condition, closure gates, and authorized approval before submission.";
  ...
  return (
    <section className="guided-workflow">
      ...
      <section className="procedure-banner">
        ...
        <p>QMSPilot manufacturing baseline. Customer terminology, routing, data sources, response times, authorities, risk thresholds, and procedure references remain configurable during onboarding.</p>
      </section>

      <section className="stage-strip">
        {tool.stages.map((stageName, index) => <button key={stageName} className={index === stage ? "active" : index < stage ? "complete" : ""} onClick={() => setStage(index)}><span>{index < stage ? "✓" : index + 1}</span><strong>{stageName}</strong></button>)}
      </section>

      <OperationsFunctionPanel toolId={tool.id} values={values} dueDate={due} setValue={setValue} />

      <div className="guided-layout">
        <div className="guided-form-column">
          <article className="guided-card">
            <div className="guided-title"><div><small>RECORD CONTROL</small><h2>Identity, timing, and accountability</h2></div><FileText /></div>
            <div className="record-grid">
              ...
              <label>Record date *<input type="date" value={meta.eventDate} onChange={event => setMeta({ ...meta, eventDate: event.target.value })} /></label>
            ...
            return sectionFields.length ? (
              <article className="guided-card" key={section}>
                ...
                <div className="field-grid">
                  {sectionFields.map(field => <OperationalField key={field.key} field={field} value={values[field.key] || ""} other={values[`${field.key}_other`] || ""} change={value => setValue(field.key, value)} otherChange={value => setValue(`${field.key}_other`, value)} />)}
                </div>
              </article>
            ) : null;
          })}
        ...
        <aside className="guided-side">
          <article className="guided-card evidence-card">
            ...
            <div className={`evidence-requirement ${tool.evidenceRequired ? "required" : "optional"}`}><strong>{tool.evidenceRequired ? "Required before submission" : "Available when it strengthens the record"}</strong><span>Capture the actual condition, data source, response, verification, or authorization.</span></div>
            ...
            <div className="evidence-buttons rich">
              ...
              <input hidden ref={visionRef} type="file" accept="image/*" capture="environment" onChange={(event: ChangeEvent<HTMLInputElement>) => addEvidence("vision", event.target.files, "AI-assisted review queued; qualified human verification remains required") } />
            </div>
            {evidence.length > 0 && <div className="evidence-list">{evidence.map(item => <div key={item.id}>{item.url && ["photo", "vision", "signature"].includes(item.kind) ? <img src={item.url} alt="Operational evidence" /> : <span>{item.kind === "video" ? <Video /> : item.kind === "audio" ? <Play /> : item.kind === "location" ? <MapPin /> : item.kind === "scan" ? <Barcode /> : <FileText />}</span>}<p><strong>{item.name}</strong><small>{item.kind.toUpperCase()} · {bytes(item.size)}</small>{item.note && <em>{item.note}</em>}</p><button onClick={() => setEvidence(current => current.filter(candidate => candidate.id !== item.id))}><Trash2 /></button></div>)}</div>}
          </article>

          <article className="guided-card gate-card">
            <div className="guided-title"><div><small>CLOSURE GATES</small><h2>Operational controls</h2></div><ShieldCheck /></div>
          ...
          <article className="guided-card approval-card">
            ...
            <label>Decision *<select value={decision} onChange={event => setDecision(event.target.value)}><option value="">Select</option><option>Approved</option><option>Approved with conditions</option><option>Returned for correction</option><option>Rejected</option></select></label>
            {decision === "Approved with conditions" && <label>Conditions *<textarea value={conditions} onChange={event => setConditions(event.target.value)} /></label>}
          </article>

          <article className="guided-card ai-card"><Sparkles size={18} /><strong>Pilot operational guidance</strong><p>{guidance}</p><small>Pilot may summarize evidence, identify data gaps, calculate operational metrics, and recommend escalation. Qualified people retain production, safety, quality, release, and resource authority.</small></article>

          <article className="guided-card escalation-card">
            <div className="guided-title"><div><small>ESCALATION RULES</small><h2>When this must move</h2></div><Target /></div>
            {tool.escalations.map(item => <div key={item.when}><strong>{item.when}</strong><span>{item.route}</span></div>)}
          </article>
        </aside>
      </div>
      ...
      {signature && <SignatureModal onClose={() => setSignature(false)} onSave={(url, size) => { setEvidence(current => [...current, { id: uid(), kind: "signature", name: `operations-signature-${today()}.png`, size, url }]); setSignature(false); setNotice("Electronic signature attached."); }} />}
    </section>
  );
}
