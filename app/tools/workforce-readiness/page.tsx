...
type Setup = {
  ...
  leadershipIntent: string;
};

type Selection = { personId: string; capabilityKey: string } | null;

const draftKey = "qmspilot:workforce-readiness:draft";
const recordsKey = "qmspilot:workforce-readiness:records";

const levelMeta: Record<Level, { label: string; short: string; status: string }> = {
  0: { label: "Not assigned", short: "0", status: "not_assigned" },
  1: { label: "Training required", short: "1", status: "training_required" },
  2: { label: "In training", short: "2", status: "in_training" },
  3: { label: "Qualified with supervision", short: "3", status: "qualified_supervised" },
  4: { label: "Fully qualified", short: "4", status: "fully_qualified" },
  5: { label: "Trainer / subject-matter expert", short: "5", status: "trainer" },
};
...
function blankQualification(level: Level = 0): Qualification {
  return { level, evaluator: "", effectiveDate: "", reviewDate: "", restriction: "", evidenceNote: "", evidenceNames: [] };
}

function qualificationMap(levels: Level[]): Record<string, Qualification> {
...
export default function WorkforceReadinessPage() {
  ...
  useEffect(() => {
    ...
    if (!saved) return;
    try {
      const draft = JSON.parse(saved) as { setup?: Setup; people?: Person[]; recordId?: string };
  ...
  const metrics = useMemo(() => {
    const qualifications = people.flatMap((person) => capabilities.map((capability) => ({ person, capability, qualification: person.qualifications[capability.key] ?? blankQualification() })));
    const levelPoints = qualifications.reduce((sum, item) => sum + item.qualification.level, 0);
    const readinessScore = qualifications.length ? Math.round((levelPoints / (qualifications.length * 5)) * 100) : 0;
    ...
    return { readinessScore, criticalGaps, singlePointDependencies, inTraining, expiring, fullyQualified, trainers, qualifications };
  }, [people]);
  ...
  function updateQualification(personId: string, capabilityKey: string, patch: Partial<Qualification>) {
  ...
  function attachEvidence(event: ChangeEvent<HTMLInputElement>) {
    if (!selected) return;
  ...
  function buildPayload(id: string) {
    return {
      ...
      recommendations,
      governance: {
        humanQualificationAuthority: true,
        objectiveEvidenceRequired: true,
        source: "Northstar Workforce Readiness",
      },
    };
  }
  ...
  async function submitToNorthstar() {
    ...
    try {
      ...
      if (cloud.status === "ready" && cloud.organizationId && cloud.user) {
        ...
        const personIds = new Map((savedPeople ?? []).map((person: { id: string; employee_code: string }) => [person.employee_code, person.id]));
        ...
        const qualificationRows = people.flatMap((person) => capabilities.map((capability) => {
          ...
          return {
            organization_id: cloud.organizationId,
            snapshot_id: snapshot.id,
            person_id: personIds.get(person.employeeCode),
    ...
    <main className="wr-shell">
          ...
          <div className="matrix-wrap">
            <table>
              ...
              <tbody>{people.map((person) => <tr key={person.id}><td><strong>{person.name}</strong><small>{person.role} · {person.shift}</small></td>{capabilities.map((capability) => {
                const qualification = person.qualifications[capability.key] ?? blankQualification();
                return <td key={capability.key}><button className={`level level-${qualification.level}`} onClick={() => setSelection({ personId: person.id, capabilityKey: capability.key })}><b>{qualification.level}</b><span>{levelMeta[qualification.level].short}</span></button><select aria-label={`${person.name} ${capability.name}`} value={qualification.level} onChange={(event) => updateQualification(person.id, capability.key, { level: Number(event.target.value) as Level })}>{([0,1,2,3,4,5] as Level[]).map((level) => <option value={level} key={level}>{level} · {levelMeta[level].label}</option>)}</select></td>;
              })}</tr>)}</tbody>
            </table>
          </div>
        )}
        <div className="legend">{([0,1,2,3,4,5] as Level[]).map((level) => <span key={level}><b className={`level-dot level-${level}`}>{level}</b>{levelMeta[level].label}</span>)}</div>
      </section>
      ...
      {selected && <section className="panel qualification-panel">
        ...
        <div className="qualification-grid">
          <label>Qualification level<select value={selected.qualification.level} onChange={(event) => updateQualification(selected.person.id, selected.capability.key, { level: Number(event.target.value) as Level })}>{([0,1,2,3,4,5] as Level[]).map((level) => <option value={level} key={level}>{level} · {levelMeta[level].label}</option>)}</select></label>
      ...
      <section className="two-grid">
        <article className="panel risk-panel">
          ...
          <div className="risk-list">
            {capabilities.filter((item) => item.critical).map((capability) => {
        ...
        <article className="panel plan-panel">
          ...
          <div className="plan-list">
            {recommendations.length ? recommendations.map((item) => <div key={item.capability}><span className={item.priority.toLowerCase()}>{item.priority}</span><span><strong>{item.capability}</strong><small>Develop {item.candidate} from level {item.currentLevel} · Current full coverage: {item.coverage}</small></span></div>) : <div className="success-state"><CheckCircle2 /><span><strong>Critical coverage is controlled.</strong><small>Continue planned qualification renewals and succession development.</small></span></div>}
          </div>
        </article>
      </section>
