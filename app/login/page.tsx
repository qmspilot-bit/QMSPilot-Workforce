  ...
  return (
    <main className="northstar-login-shell">
      <section className="northstar-login-brand">
        <div className="northstar-login-logo-row">
          <div className="northstar-login-logo qms"><img src={QMSPILOT_LOGO_DATA_URI} alt="QMSPilot" /></div>
          <div className="northstar-login-logo ns"><img src={NORTHSTAR_LOGO_DATA_URI} alt="Northstar" /></div>
        </div>

        <div className="northstar-login-message">
          <small>ENTERPRISE QUALITY EXECUTION</small>
          <h1>One secure identity across every Northstar workspace.</h1>
          <p>Sign in once to carry your organization, site, role, permissions, approvals, and controlled-record accountability throughout the platform.</p>
        </div>

        <div className="northstar-login-assurances">
          <div><ShieldCheck size={20} /><span><strong>Controlled access</strong><small>Identity and authorization remain separate from AI recommendations.</small></span></div>
          <div><Building2 size={20} /><span><strong>Tenant-aware context</strong><small>Northstar preserves the active company, site, and workspace.</small></span></div>
          <div><CheckCircle2 size={20} /><span><strong>Verified accountability</strong><small>Actions retain ownership, evidence, decisions, and closure history.</small></span></div>
        </div>
      ...
      <section className="northstar-login-access">
        <div className="northstar-login-card">
          <div className="northstar-login-card-head">
            <span><LockKeyhole size={21} /></span>
            <div><small>QMSPILOT NORTHSTAR</small><h2>Secure workspace access</h2></div>
          </div>

          <p className="northstar-login-intro">Use the work email assigned to your Northstar account.</p>

          <form onSubmit={handleSubmit}>
            <label>
              <span>Work email</span>
              <div className="northstar-login-field"><Mail size={17} /><input type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="name@company.com" /></div>
            </label>

            <label>
              <span>Password</span>
              <div className="northstar-login-field"><LockKeyhole size={17} /><input type={showPassword ? "text" : "password"} autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Enter your password" /><button type="button" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? <EyeOff size={16} /> : <Eye size={16} />}</button></div>
            </label>

            <div className="northstar-login-help"><a href={recoveryHref}>Forgot password?</a></div>

            {error && <div className="northstar-login-error" role="alert" aria-live="polite">{error}</div>}

            <button className="northstar-login-submit" type="submit" disabled={submitting || !supabase}>
              {submitting ? "Authenticating…" : supabase ? "Sign in to Northstar" : "Cloud authentication unavailable"}
              {!submitting && supabase && <ArrowRight size={17} />}
            </button>
          </form>
